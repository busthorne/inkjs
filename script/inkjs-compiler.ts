...
import { AstSerializer } from '../src/compiler/Parser/ParsedHierarchy/AstSerializer';
...
Usage: inkjs-compiler <options> <ink file>
   ...
   -a:              Output AST (Abstract Syntax Tree) to stdout
`);
	process.exit(0);
}
...
let outputAst = process.argv.includes("-a");
...
process.argv = process.argv.filter(p => !['-c', '-o', '-p', '-k', '-s', '-a'].includes(p));
...
if (!inputFile.endsWith(".json")) {
	...
	try {
		...
		if (outputAst) {
			const astJson = AstSerializer.serializeToJson(c.parsedStory);
			process.stdout.write(astJson + "\n");
			process.exit(0); // Exit after printing AST
		}
	} catch (error: unknown) {
	...
	if (outputAst) {
		process.stderr.write("WARNING: Could not output AST for an already compiled story. Try it on a .ink file instead." + "\n");
		outputAst = false;
	}
